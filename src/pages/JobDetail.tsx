                    ...
                    <div className={`relative transition-all duration-500 ${isPdfExpanded ? "h-[800px]" : "h-[500px]"}`}>
                      ...
                      {pdfError ? (
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted/20">
                          ...
                          <p className="text-sm text-muted-foreground mb-3">Unable to preview PDF in browser</p>
                          <Button
                            size="sm"
                            onClick={() => downloadPdf(
                              resolveFileUrl(job.jobDescriptionPdf?.url || ''),
              ...
              <div className="lg:col-span-2 space-y-6 lg:sticky lg:top-24">
                ...
                <div className="bg-card rounded-2xl p-6 border border-border/50 shadow-soft animate-fade-in" style={{ animationDelay: "200ms" }}>
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Additional Details — Multi-role tabs OR single-role list */}
                    {hasMultipleRoles ? (
                      <div className="border-t border-border/50 pt-4 mt-2">
                        <div className="flex items-center justify-between mb-3 gap-2">
                          <p className="text-xs text-muted-foreground uppercase font-medium">Additional Details</p>
                        ...
                        <Tabs defaultValue="role-0" className="w-full">
                          <TabsList className="w-full justify-start h-auto p-1 bg-muted rounded-lg mb-4 flex-wrap gap-1">
                            {job.roles!.map((role, idx) => {
                              ...
                              return (
                                <TabsTrigger
                                  key={role.id || idx}
                                  value={`role-${idx}`}
                                  className="rounded-md text-xs px-3 py-1.5 data-[state=active]:shadow-sm flex-1 min-w-fit"
                                >
                                  {tabLabel}
                                </TabsTrigger>
                              );
                          ...
                          {job.roles!.map((role, idx) => (
                            <TabsContent key={role.id || idx} value={`role-${idx}`} className="space-y-3 animate-fade-in mt-0">
                              {role.customInputs && role.customInputs.length > 0 ? (
                                ...
                                <p className="text-sm text-muted-foreground italic">No details for this role.</p>
                              )}
                            </TabsContent>
                          ))}
                        </Tabs>
